...
<table class="ug-table">
  ...
  <tr><td>Choke</td><td>e.g. <code>36/64"</code> or <code>56/67"</code></td><td>Full fraction parsed (numerator ÷ denominator). P1 is the reference — P2/P3 scale relative to P1's choke opening.</td></tr>
  <tr><td>THP (Bar)</td><td>Tubing head pressure at time of test</td><td>P1 THP is the reference. P2/P3 scale as (THP_P2 / THP_P1).</td></tr>
...
<table class="ug-table">
  ...
  <tr><td>Reservoir Gas Target (P1)</td><td><code>Q_res_P1 = Qgas_P1 / Bg</code></td></tr>
  <tr><td>Reservoir Gas Target (P2/P3)</td><td><code>Q_res_Pn = Q_res_P1 × (choke_Pn / choke_P1)² × (THP_Pn / THP_P1)</code></td></tr>
...
with st.sidebar:
    ...
    if st.button("＋ Add Survey Row"):
        ...
        if p1:
            try:
                c1_in = parse_choke(p1['choke'])
                c2_in = parse_choke('32/64"')   # default choke for new row
                if c1_in and c2_in and p1['thp'] > 0:
                    new_qgas = round(p1['qgas'] * (c2_in / c1_in)**2 * (65.0 / p1['thp']), 2)
            except Exception:
                pass
...
# ── Choke parser — works for any X/Y" format, not just X/64" ──────────────────
def parse_choke(choke_str):
    """Convert a choke string like '36/64"', '56/67"', '32/64"', or '0.5625'
    into the actual opening size in inches (float).
    Returns None if the string is not numeric (e.g. 'Closed')."""
    try:
        s = str(choke_str).replace('"', "").replace("'", "").strip()
        parts = s.split('/')
        if len(parts) == 2:
...
if run_clicked:
    ...
    try:
        ...
        # P1 is the reference test — its Q_res IS the measured rate, no scaling factor needed
        SIM_TARGET_RES = prim['qgas'] / bg_val
        S1_STABLE_REF  = prim['qgas'] / bg_val

        # Store P1 reference choke (inches) and THP for scaling P2/P3 relative to P1
        ref_choke_in = parse_choke(prim['choke'])
        ref_thp      = prim['thp']
        if ref_choke_in is None:
            st.error(f"❌  Cannot parse P1 choke value: '{prim['choke']}'. "
                     "Use format like  36/64\"  or  56/67\"")
            st.stop()
        ...
        logp(f'   P1 reference: choke = {ref_choke_in:.6f} in  |  THP = {ref_thp:.2f} Bar')
        for ft in flowing_tests:
            cn_in = parse_choke(ft['choke'])
            if cn_in is not None and ref_choke_in is not None:
                tgt = SIM_TARGET_RES * (cn_in / ref_choke_in)**2 * (ft['thp'] / ref_thp)
            else:
                tgt = ft['qgas'] / bg_val
            logp(f"   ✅ {ft['survey']:3s} | Choke={ft['choke']:8s} ({cn_in:.6f} in) | "
                 f"THP={ft['thp']:.1f} | Qgas={ft['qgas']:.0f} m³/d | "
                 f"Qoil={ft['qoil']:.3f} | Qwtr={ft['qwtr']:.3f} | Q_res={tgt:.3f} m³/d")
        ...
        for ft in flowing_tests[1:]:
            cn2_in = parse_choke(ft['choke'])
            if cn2_in is not None and ref_choke_in is not None:
                # Scale relative to P1 reference: (choke_ratio)^2 * (THP_ratio)
                q2r = SIM_TARGET_RES * (cn2_in / ref_choke_in)**2 * (ft['thp'] / ref_thp)
            else:
                q2r = ft['qgas'] / bg_val
